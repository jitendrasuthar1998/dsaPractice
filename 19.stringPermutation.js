...
function permutation(string, order) {
    let newString = [];

    let characters = [];

    for(let i=0;i<string.length;i++) {characters.push(string[i])}


    for(let i=0; i<order.length; i++) {
        newString[order[i]] = characters[i]
    }

    return newString.join("");
}

console.log(permutation("abcd", [2,4,3,1]));